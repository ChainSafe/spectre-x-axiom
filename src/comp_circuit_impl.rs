...
use axiom_eth::{
    ...
    utils::{
        ...
        component::{
            circuit::{
                ComponentBuilder, CoreBuilder, CoreBuilderOutput, CoreBuilderParams, PromiseBuilder,
            },
...
#[derive(Clone, Debug)]
pub struct ComponentCircuitImpl<
    F: Field,
...
{
    pub rlc_builder: RefCell<ShaCircuitBuilder<F, ShaFlexGateManager<F>>>,
    ...
    <C as ComponentBuilder<F>>::Params: CoreBuilderParams,
{
    ...
    pub fn new(
        ...
        prompt_rlc_params: BaseCircuitParams,
    ) -> Self {
    ...
    pub fn new_from_stage(
        ...
        let mut circuit_builder =
            ShaCircuitBuilder::from_stage(stage).use_params(prompt_rlc_params);
        // Public instances are fully managed by ComponentCircuitImpl.
        circuit_builder.base_mut().set_instance_columns(1);
        Self {
            rlc_builder: RefCell::new(circuit_builder),
    ...
    pub fn virtual_assign_phase0(&self) -> Result<(), Error> {
        ...
        let pis = &mut rlc_builder.base_mut().assigned_instances;
    ...
    fn generate_public_instances(
        &self,
        rlc_builder: &mut ShaCircuitBuilder<F, ShaFlexGateManager<F>>,
        ...
        let ctx = rlc_builder.base_mut().main(0);
...
where
    <C as ComponentBuilder<F>>::Params: CoreBuilderParams,
{
    type Config = (C::Config, P::Config, SHAConfig<F, SpreadConfig<F>>);
    type Params = (C::Params, P::Params, BaseCircuitParams);
    ...
    fn configure_with_params(meta: &mut ConstraintSystem<F>, params: Self::Params) -> Self::Config {
        ...
        let mut rlc_config = SHAConfig::configure(meta, params.2.clone());
